...
class DeepSeekBaziReport:
    ...
    def _call_deepseek_api_sync(self, prompt: str, model: str = "deepseek-chat", temperature: float = 0.7) -> str:
        ...
        system_prompt_content = """你是一位精通中华传统八字命理学的资深专家。在生成报告时，请始终采用娓娓道来的叙述风格，确保语言流畅自然，富有亲和力。同时要尽量做到客观，不要只输出好的方面，也要有坏的提醒。对于每一个分析要点，都请用3到5句完整且连贯的句子组成一个流畅的小段落进行深入浅出的阐释，让不了解八字的用户也能轻松理解。
            """
        data = {
            "model": model,
            "messages": [
    ...
    async def generate_bazi_analysis_module_async(self, bazi_str: str, gender: str, age_info: str) -> str: # ADDED age_info
        prompt = f"""
            ...
            **重要提示：** 请您在撰写此模块时，采用娓娓道来的叙述风格。同时要尽量做到客观，不要只输出好的方面，也要有坏的提醒。对于以下列出的每一个主要分析点（如“1. 四柱干支”、“2. 五行力量分析”等）及其内部的子项（若有），请不要只是简单罗列信息。而是针对每一项，都用3到5句完整且连贯的句子组成一个流畅的小段落来进行深入浅出的阐释。您的目标是让即使不熟悉八字命理的读者也能轻松理解各项分析的含义、逻辑及其对命主的影响。请保持专业的分析深度，同时确保语言通俗易懂、富有亲和力。请直接以这样的叙述方式填充各个要点，并使用Markdown格式化您的输出，包括标题和必要的强调。
    ...
    async def generate_mingge_decode_module_async(self, bazi_str: str, gender: str, age_info: str) -> str: # ADDED age_info
        prompt = f"""
            ...
            **重要提示：** 请您在撰写此模块时，采用娓娓道来的叙述风格。同时要尽量做到客观，不要只输出好的方面，也要有坏的提醒。对于以下列出的每一个主要分析点及其内部的子项（若有），请不要只是简单罗列信息。而是针对每一项，都用3到5句完整且连贯的句子组成一个流畅的小段落来进行深入浅出的阐释。您的目标是让即使不熟悉八字命理的读者也能轻松理解各项分析的含义、逻辑及其对命主的影响。请保持专业的分析深度，同时确保语言通俗易懂、富有亲和力。请直接以这样的叙述方式填充各个要点，并使用Markdown格式化您的输出。
    ...
    async def generate_fortune_flow_module_async(self, bazi_str: str, gender: str, year: int, month: int, day: int, hour: int, age_info: str) -> str: # ADDED age_info
        ...
        prompt = f"""
            ...
            **重要提示：** 请您在撰写此模块时，采用娓娓道来的叙述风格，注意采用与命主年龄相匹配的叙述。同时要尽量做到客观，不要只输出好的方面，也要有坏的提醒。对于以下列出的每一个主要分析点及其内部的子项（若有），请不要只是简单罗列信息。而是针对每一项，都用3到5句完整且连贯的句子组成一个流畅的小段落来进行深入浅出的阐释。您的目标是让即使不熟悉八字命理的读者也能轻松理解各项分析的含义、逻辑及其对命主的影响。请保持专业的分析深度，同时确保语言通俗易懂、富有亲和力。请直接以这样的叙述方式填充各个要点，并使用Markdown格式化您的输出。
    ...
    def _calculate_dayun(self, year, month, day, hour, minute=0, second=0, sex="男"):
        ...
        try:
            ...
            age_info_internal = f"{age}周岁" if age >=0 and age <= 90 else "暂无"
